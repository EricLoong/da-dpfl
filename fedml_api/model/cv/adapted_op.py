...
class AdaptedLeafCNN1(nn.Module):
    ...
    def adapted_forward(self, x):
        ...
        x = self.pool(
            F.relu(
                self.conv1._conv_forward(
                    x,
                    weight=self.adapted_model_para["conv1.weight"],
                    bias=self.adapted_model_para["conv1.bias"],
                )
            )
        )
        x = self.pool(
            F.relu(
                self.conv2._conv_forward(
                    x,
                    weight=self.adapted_model_para["conv2.weight"],
                    bias=self.adapted_model_para["conv2.bias"],
                )
            )
        )
        x = x.view(-1, 64 * 4 * 4)
        x = F.relu(
            F.linear(
                x,
                weight=self.adapted_model_para["fc1.weight"],
                bias=self.adapted_model_para["fc1.bias"],
            )
        )
        x = F.relu(
            F.linear(
                x,
                weight=self.adapted_model_para["output.weight"],
                bias=self.adapted_model_para["output.bias"],
            )
        )
        return x
...
class AdaptedLeNet(AdaptedLeafCNN1):
    ...
    def __init__(
        self, num_classes, n_kernels=32, in_channels=3, fc_factor=1, fc_factor2=1
    ):
    ...
    def adapted_forward(self, x):
        ...
        x = F.relu(
            F.linear(
                x,
                weight=self.adapted_model_para["fc2.weight"],
                bias=self.adapted_model_para["fc2.bias"],
            )
        )
        x = F.relu(
            F.linear(
...
def map_module_name(name):
    mapped_name = [
        f"_modules.[{m_name}]" if m_name.isnumeric() else m_name
