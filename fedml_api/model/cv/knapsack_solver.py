...
class KnapsackSolver01(object):
    ...
    def reset_state(
        self, value_sum_max=0, item_num_max=0, weight_max=0, iter_version=False
    ):
    ...
    def found_max_value(self, weight_list, value_list, capacity):
        ...
        self.reset_state(
            value_sum_max=value_sum_max,
            item_num_max=len(weight_list),
...
class KnapsackSolverFractional(object):
    ...
    def _found_max_value_numeric(self, weight_list, value_list, capacity):
        ...
        return (
            totalValue,
            self.selected_item,
            self.selected_item / np.array(weight_list),
        )
    ...
    def found_max_value(self, weight_list, value_list, capacity):
        ...
        tensor_type = isinstance(weight_list, torch.Tensor) and isinstance(
            value_list, torch.Tensor
        )
        normal_numeric_type = isinstance(
            weight_list[0], (int, float, complex)
        ) and isinstance(value_list[0], (int, float, complex))
        assert (
            tensor_type or normal_numeric_type
