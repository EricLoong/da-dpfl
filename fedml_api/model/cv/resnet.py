...
def customized_resnet50(
    pretrained: bool = False, class_num=100, progress: bool = True
) -> ResNet:
    res50 = ResNet(BasicBlock, [3, 4, 6, 3], class_num=class_num)

    # Change BN to GN for res50
    res50.bn1 = nn.GroupNorm(num_groups=32, num_channels=64)
    for layer in [res50.layer1, res50.layer2, res50.layer3, res50.layer4]:
        for block in layer:
            block.bn1 = nn.GroupNorm(
                num_groups=32, num_channels=block.conv1.out_channels
            )
            block.bn2 = nn.GroupNorm(
                num_groups=32, num_channels=block.conv2.out_channels
            )

            # Safely replace BatchNorm with GroupNorm in the shortcut
            if (
                hasattr(block, "shortcut")
                and isinstance(block.shortcut, nn.Sequential)
                and len(block.shortcut) > 1
            ):
                block.shortcut[1] = nn.GroupNorm(
                    num_groups=32, num_channels=block.shortcut[0].out_channels
                )

    assert len(dict(res50.named_parameters()).keys()) == len(
        res50.state_dict().keys()
    ), "More BN layers are there..."
