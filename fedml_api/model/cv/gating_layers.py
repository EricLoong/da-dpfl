...
from fedml_api.model.cv import switchable_norm
from fedml_api.model.cv.adapted_op import AdaptedLinear, deepgetattr
from fedml_api.standalone.fedgate.utils.constants import (
    IN_PLANES_TYPE,
    SHAKESPEARE_CONFIG,
)
from fedml_api.model.cv.adapted_op import map_module_name
    ...
    def forward(self, x):
        if self.reshape_type == "flat_dim0":
            ...
            raise NotImplementedError(
                "Un-supported reshape_type: {}".format(self.reshape_type)
            )
...
class GatingLayer(nn.Module):
    def __init__(
        ...
        ori_block_size_lookup_table = torch.tensor(
            [p.numel() for p in model_to_mask.parameters()], device=device
        )
        ...
        assert (
            self.fine_grained_block_split >= 1
        ), f"fine_grained_block_split must >= 1, while got {fine_grained_block_split}"
        if self.fine_grained_block_split == 1:
            ...
            for i, name in enumerate(ori_block_names):
                ...
                size_each_sub = torch.ceil(
                    ori_block_size_lookup_table[i] / split_num
                ).int()
                # e.g., split_num = ceil(25/3) = 9
                split_num = torch.ceil(
                    ori_block_size_lookup_table[i] / size_each_sub
                ).int()
                self.para_name_to_block_split_info[name] = (
                    cur_block_idx,
                    cur_block_idx + split_num,
                    size_each_sub,
                )
                ...
                size_last_sub = (
                    ori_block_size_lookup_table[i] - (split_num - 1) * size_each_sub
                )
                self.block_size_lookup_table.append(size_last_sub)
                if i in linear_layer_block_idx:
                    self.linear_layer_block_idx.append(cur_block_idx)
                cur_block_idx += 1
        self.non_linear_layer_block_idx = [
            i
            for i in range(len(self.block_size_lookup_table))
        ...
        block_size_lookup_table_linear = self.block_size_lookup_table[
            self.linear_layer_block_idx
        ].double()
        block_size_lookup_table_non_linear = self.block_size_lookup_table[
            self.non_linear_layer_block_idx
        ].double()
        ...
        self.block_size_lookup_table_normalized = deepcopy(
            self.block_size_lookup_table
        ).double()
        self.block_size_lookup_table_normalized[
            self.linear_layer_block_idx
        ] = block_size_lookup_table_linear
        self.block_size_lookup_table_normalized[
            self.non_linear_layer_block_idx
        ] = block_size_lookup_table_non_linear

        # ---------------------------- Build gating layer ----------------------------

        norm_input_layer = switchable_norm.SwitchNorm2d(IN_PLANES_TYPE[dataset_name])
        input_feat_size = 3 * 32 * 32

        reshape_layer = Reshape(reshape_type="flat_dim0")
        output_layer = nn.Linear(
            in_features=input_feat_size, out_features=len(self.block_names)
        )
        norm_outputs = torch.nn.BatchNorm1d(num_features=len(self.block_names))
        self.norm_input_layer = norm_input_layer

        self.gating = nn.Sequential(reshape_layer, output_layer, norm_outputs)
    ...
    def forward(self, x):
        assert len(x.size()) in [
            3,
            4,
        ], f"Un-expected input shape for gating layer, got {len(x.size())}"
        if self.norm_input_each_forward:
            x = self.norm_input_layer(x)
        # input shape is 105, 50 when batch_size is 128
        ## return the score of every feature (number of columns is the feature num)
        gating_score = self._forward(x, self.gating)

        if self.seperate_trans == 1:
            trans_weight = self._forward(x, self.w_transform)
        else:
            # same shape and value in this condition
    ...
    def _forward(self, x, layer):
        ...
        if len(res.size()) == 3:  # [B, seq_len, block_len] for texts
            ...
            raise RuntimeError(
                f"Un-expected mask weights shape for gating layer, got {len(res.size())}"
            )
