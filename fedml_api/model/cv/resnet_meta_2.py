...
class first_conv_block(nn.Module):
    ...
    def forward(self, x, oup_scale_id):
        ...
        out = F.conv2d(
            x, conv1_weight[:oup, :, :, :], bias=None, stride=self.stride, padding=3
        )
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, mid_scale_id, inp_scale_id, oup_scale_id):
        ...
        scale_ratio_tensor = torch.FloatTensor([mid_scale, inp_scale, oup_scale]).to(
            x.device
        )
        ...
        out = F.conv2d(
            x, conv1_weight[:mid, :inp, :, :], bias=None, stride=1, padding=0, groups=1
        )
        ...
        if self.is_downsample:
            ...
            conv1_downsample_weight = self.fc12_downsample(fc11_downsample_out).view(
                self.max_oup, self.max_inp, 1, 1
            )

            identity = F.conv2d(
                x,
                conv1_downsample_weight[:oup, :inp, :, :],
                bias=None,
                stride=self.stride,
                padding=0,
                groups=1,
        ...
        for i, block in enumerate(self.layers):
            if i == 0:
                ...
                x = block(
                    x,
                    mid_scale_ids[i - 1],
                    stage_oup_scale_ids[i - 1],
                    stage_oup_scale_ids[i],
                )
