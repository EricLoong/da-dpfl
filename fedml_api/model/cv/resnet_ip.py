...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1_g._conv_forward(
            x, weight=self.conv1_g.weight + self.conv1_v.weight
        )
        out = self.bn1_g.forward(
            out,
            weight=self.bn1_g.weight + self.bn1_v.weight,
            bias=self.bn1_g.bias + self.bn1_v.bias,
        )

        out = self.relu(out)

        out = self.conv2_g._conv_forward(
            out, weight=self.conv2_g.weight + self.conv2_v.weight
        )
        out = self.bn2_g.forward(
            out,
            weight=self.bn2_g.weight + self.bn2_v.weight,
            bias=self.bn2_g.bias + self.bn2_v.bias,
        )
        out = self.relu(out)
        out = self.conv3_g._conv_forward(
            out, weight=self.conv3_g.weight + self.conv3_v.weight
        )
        out = self.bn3_g.forward(
            out,
            weight=self.bn3_g.weight + self.bn3_v.weight,
            bias=self.bn3_g.bias + self.bn3_v.bias,
        )

        if self.downsample is not None:
            identity = self.downsample.conv1_g._conv_forward(
                x,
                weight=self.downsample.conv1_g.weight + self.downsample.conv1_v.weight,
            )
            identity = self.downsample.bn1_g.forward(
                identity,
                weight=self.downsample.bn1_g.weight + self.downsample.bn1_v.weight,
...
class ResNet_ip(nn.Module):
                ...
                OrderedDict(
                    [
                        ...
                        (
                            "bn1_g",
                            norm_layer(
                                planes * block.expansion, track_running_stats=False
                            ),
                        ),
                        (
                            "conv1_v",
    ...
    def forward(self, x):
        ...
        x = self.conv1_g._conv_forward(
            x, weight=self.conv1_g.weight + self.conv1_v.weight
        )

        x = self.bn1_g.forward(
            x,
            weight=self.bn1_g.weight + self.bn1_v.weight,
            bias=self.bn1_g.bias + self.bn1_v.bias,
        )
        ...
        x = F.linear(
            x_f,
            weight=self.fc_g.weight + self.fc_v.weight,
