...
def _data_transforms_cifar10():
    ''' build a series of transforms together for data preprocessing

    '''
    CIFAR_MEAN = [0.49139968, 0.48215827, 0.44653124]
    CIFAR_STD = [0.24703233, 0.24348505, 0.26158768]
    # compose serveral transforms together
...
def load_cifar10_data(datadir):
    ''' using defined data transformers to generate desired data and format
...
def partition_data(datadir, partition, n_nets, alpha, logger):
    ...
    :param datadir: the root folder of dataset
    :param partition: the way of data distribution: dirichlet(dir), 
            n_cls(partial classes for per client), 
            my_part(partition all cilents into PA shards)
    :param n_nets: the number of clients
    :param alpha: the parameter to control data distribution
    :param logger: the logging object

    return: split train, test data, the mapping between client and data, traindata_cls_counts
...
def get_dataloader_cifar10(datadir, train_bs, test_bs, dataidxs=None, test_idxs=None, cache_train_data_set=None,
                           cache_test_data_set=None, logger=None):
    ...
    train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=False, num_workers=0, pin_memory=True)
    test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, drop_last=False, num_workers=0, pin_memory=True)
    return train_dl, test_dl


#def get_raw_dataloader_cifar10(datadir, train_bs, dataidxs=None, cache_train_data_set=None, logger=None):
#    _, transform_raw_train = _data_transforms_cifar10()
#    # CKA comparing data sub-scaling
#    dataidxs = np.random.choice(dataidxs, int(len(dataidxs)*0.2), replace=False)
#    logger.info("raw_train_num{}".format(len(dataidxs)))
#    train_ds = CIFAR10_truncated(datadir, dataidxs=dataidxs, train=True, transform=transform_raw_train, download=True,
#                                  cache_data_set=cache_train_data_set)
